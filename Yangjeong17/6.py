...
# 2) 평균 점수
total = sum([score for values, score in students])
avg = total / count
print("평균:", avg)

# 3) 최고점(동점 포함)
top_score = max([score for values, score in students])
...
# 5) 60점 미만 존재 여부
has_fail = any([score < 60 for values, score in students])
print("60점 미만 존재?", has_fail)

# 6) 전원 합격 여부
all_pass = all([score >= 60 for values, score in students])
