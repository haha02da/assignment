...
# 3) 최고점(동점 포함)
top_score = max([score for name, score in students])
top_students = list(
    filter(lambda x: x[1] == top_score, students)
)
print("최고점:", top_score)
...
# 5) 60점 미만 존재 여부
has_fail = any([score < 60 for name, score in students])
print("60점 미만 존재?", has_fail)

# 6) 전원 합격 여부
all_pass = all([score >= 60 for name, score in students])
